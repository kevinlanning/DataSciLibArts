...
What follows is part of the code I used in a recent talk about asymmetries in set sizes [@lanning2018data].

### Setup

In the first block, I load libraries, generate palette. Palette for Venn is a little weird as must be (a) visible for colorblind, (b) intersections of colors should be distinguishable, and (c) not ugly. So I fiddled here.
...
This examines the reciprocal predictability of two personality measures, the California Psychological Inventory and the California Q-set.
```{r}
...
Here, I generate proportional Venn for scholarly communities. First for personality v social. Counts are n members in Lanning (2018) Collabra
```{r}

    # order of sets is shifted to put
    # earliest level at 12 o'clock
    personality <- 12
    self_reg <- 7
    attitudes <- 13
...
Obviously, this is only the beginning of a discussion of data visualizations.  @healy2018viz addresses the limitations of infographics, in which the design stresses style over substance, and includes a rationale for a **grammar of graphics,** as instantiated in the ggplot package in R that we will be working with in the next chapter [@wickham2016r]. @cleveland1985graphical examines graphs from a more rigorous psychological and empirical viewpoint.  The [Data Stories podcasts](http://datastori.es/) are often excellent, despite the challenge of an auditory medium for visual display. I discussed some of my own data visualizations in a recent presentation which you can find [here](https://osf.io/p8tfb/) [@lanning2018data].
